...
# ── 가격 (FDR, 상폐 정량화) ──────────────────────────────
def load_prices(tickers: set[str], start: str, end: str, with_volume: bool = False):
    """선정된 적 있는 종목만 FDR 다운로드 (증분 pkl 캐시).
    반환: (opens, closes, missing) — missing = 시세 자체가 없는 종목(대부분 상폐).
    with_volume=True 면 (opens, closes, missing, volumes). 캐시는 항상 거래량까지 담아
    한 번만 받는다 — CANSLIM의 S2(거래량 2배 돌파)가 거래량을 쓰기 때문."""
    cache = CACHE_DIR / f"vbt_px2_{start}_{end}.pkl"
    data = {}
    if cache.exists():
    ...
    volumes = pd.DataFrame({t: d["volume"] for t, d in have.items() if "volume" in d.columns})
    return opens, closes, missing, volumes
